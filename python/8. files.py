...
# Define the absolute folder and CSV file path, and the output JSON file path
folder_name = 'example'  # Folder name
csv_file_path = os.path.join(os.getcwd(), folder_name, 'example.csv')  # Absolute path to the CSV file
json_file_path = os.path.join(os.getcwd(), folder_name, 'example.json')  # Absolute path for JSON output
...
try:
    with open(csv_file_path, 'r') as csvfile:
        reader = csv.DictReader(csvfile)  # Automatically uses the first row as the keys
        for row in reader:
            data.append(row)
...
def write_log(message, log_file='logfile.txt'):
    """Function to write a log message to a file with a timestamp."""
