...
amount = calculate_compound_interest(principal, rate, times_per_year, years)
print(f"The accumulated amount after {years} years is: {amount:.2f}")

# Assignment 2: Create a script that prints the current time and updates every second.
import time

# Function to print the current time every second
def display_time():
    try:
        while True:
            current_time = time.strftime("%H:%M:%S")  # Get the current time
            print(current_time)  # Print current time on a new line
